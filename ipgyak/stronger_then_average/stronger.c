...
int main()
{
    ...
    for (int i = 0; i < numberOfCompetitions; i++)
    {
        //  printf("a %d versenyen %d diak indult, minimalis pontszam: %d! Az atlag %0.2f. ", i, competitions[i].numberOfRunners, competitions[i].minResult, competitions[i].average);
        // for (int j = 0; j < competitions[i].numberOfRunners; j++)
        // {
        //   printf("%d diak - %d pontot szerzett, ", competitions[i].results[j].id, competitions[i].results[j].points);
        // }
        // printf("\n");
